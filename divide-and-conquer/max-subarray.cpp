...
// the left and find the point where we can make the max profit and do the same
// for
...
// You must go through the whole left/right half to find out.
//
// Once we have the max array from the left half, the max array from the right
// half
// from the two recursions, and we have the cross-midpoint subarray, we
// just need to compare the max sum of the three and choose the true maximum sum
// subarray!
//
...
// Later we can modify the procedures to support iterators and use std c++
// algorithms as an exercise.
//
...
decltype(auto) find_max_crossing_subarray(const vector<int> &v, int low,
                                          int mid, int high) {
  ...
  for (auto i = mid; i >= low; --i) {
    sum += v[i]; // accumulate the sum (accumulate you say? that reminds me of
                 // something...)
    if (sum > left_sum) {
      left_sum = sum;
      leftest = i;
...
decltype(auto) find_max_subarray(const vector<int> &v, int low, int high) {
  ...
  auto max_subarray = (get<2>(left_max_subarray) > get<2>(right_max_subarray))
                          ? left_max_subarray
                          : right_max_subarray;
  max_subarray = (get<2>(max_subarray) > get<2>(max_crossing_subarray))
                     ? max_subarray
...
int main() {
  ...
  // max subarray                                                 |  |    |  |
  vector<int> price_changes = {13, -3, -25, 20, 3,   -3, -16, -23, 18,
                               20, -7, 12,  -5, -22, 15, -4,  7};
  // the max profit strategy should be buy one share a day before day 8 (price
  // change 18)
  // sell all after day 11 (price change 12)
  auto max_subarray =
      find_max_subarray(price_changes, 0, price_changes.size() - 1);
  cout << get<0>(max_subarray) << "," << get<1>(max_subarray) << ","
       << get<2>(max_subarray) << "\n";
